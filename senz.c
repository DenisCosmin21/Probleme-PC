...
void show_data_senzor(uint16_t year, uint8_t month, Register_value *valori, int nr_inregistrari, int index, int *count)
{
  for(int i = 0;i < nr_inregistrari;i++){
    if(valori[i].data_senzor.year == year && valori[i].data_senzor.month == month){
      *count += 1;
      printf("%d : ", index);
...
void search_by_month_year(Sensor_list *senzori)
{
  ...
  scanf("%hhd", &luna);
  for(int i = 0;i < 5;i++){
    show_data_senzor(an, luna, senzori[i].inregistrari, senzori[i].nr_inregistrari, i, &count);
  }
  if(count == 0){
...
{
  senzor.inregistrari = realloc(senzor.inregistrari, senzor.nr_inregistrari * sizeof(Register_value));
  senzor.inregistrari[senzor.nr_inregistrari - 1] = add_data(buffer, date, senzor.inregistrari[senzor.nr_inregistrari - 1]);
...
Sensor_list *read_file(char *buffer_path, Sensor_list *senzori)
{
  ...
  for(int i = 0;i < element_number;i++){
    ...
    senzori[index].inregistrari = retrieve_sensor(buffer, senzori[index], date);
  }
  fclose(buffer);
  return senzori;
}
...
{
  for(int i = 0;i < 5;i++){
    senzori[i].status = get_state(senzori[i].inregistrari, senzori[i].nr_inregistrari);
  }
}
